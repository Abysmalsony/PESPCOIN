...
const char * const BITCOIN_CONF_FILENAME = "PESPCOIN.conf";
const char * const BITCOIN_PID_FILENAME = "PESPCOINd.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "PESPCOIN" is a composite category enabling all PESPCOIN-related debug output
                if(ptrCategory->count(std::string("PESPCOIN"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\PESPCOINCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\PESPCOINCore
    // Mac: ~/Library/Application Support/PESPCOINCore
    // Unix: ~/.PESPCOINcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "PESPCOINCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/PESPCOINCore";
#else
    // Unix
    return pathRet / ".PESPCOINcore";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty PESPCOIN.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override PESPCOIN.conf
