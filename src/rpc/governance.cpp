...
UniValue gobject(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp  ||
        ...
        throw std::runtime_error(
                ...
                "  vote-conf          - Vote on a governance object by masternode configured in PESPCOIN.conf\n"
    ...
    */
    ...
    if(strCommand == "vote-conf")
    {
        ...
        if(!fMnFound) {
            ...
            resultsObj.push_back(Pair("PESPCOIN.conf", statusObj));
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    /* PESPCOIN features */
    { "PESPCOIN",               "getgovernanceinfo",      &getgovernanceinfo,      true,  {} },
    { "PESPCOIN",               "getsuperblockbudget",    &getsuperblockbudget,    true,  {"index"} },
    { "PESPCOIN",               "gobject",                &gobject,                true,  {} },
    { "PESPCOIN",               "voteraw",                &voteraw,                true,  {} },
