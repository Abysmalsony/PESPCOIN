...
#if defined(NDEBUG)
# error "PESPCOIN Core cannot be compiled without assertions."
#endif
...
//    return fSuperblockPartOnly ? nSuperblockPart : nSubsidy - nSuperblockPart;
	...
	else if(nPrevHeight < 7899000) // max supply 7900000000
		nSubsidy = 1000 * COIN;
	else
		nSubsidy = 0;
...
void ThreadScriptCheck() {
    RenameThread("PESPCOIN-scriptch");
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck = false)
{
    ...
    // PESP : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(PESP): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(*block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(std::make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(PESP): couldn't find masternode or superblock payments"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // PESP : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        LogPrintf("CheckBlock(PESP): spork is off, skipping transaction locking checks\n");
    }
